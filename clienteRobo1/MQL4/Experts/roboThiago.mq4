...
int start(){
   ...
   int arquivo = FileOpen("mediaMovel.txt", FILE_CSV|FILE_READ, ';');
   FileReadDouble(arquivo, mediaMovel  = DOUBLE_VALUE);
   FileClose(arquivo);

   if( mediaMovel == Open[0] ){
      Abresell = true; 
   }
   if( mediaMovel == Close[0] ){
      Abrebuy = true;
   }
